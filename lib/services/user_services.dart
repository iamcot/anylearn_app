...
class UserService extends BaseService {
  ...
  Future<ProfileDTO> getProfile(String token, int page) async {
    final url =
        buildUrl(appConfig: config, endPoint: "/social/profile", token: token, query: page > 1 ? "page=$page" : "");
    final json = await get(httpClient, url);
    print(url);
  ...
  Future<ProfileDTO> getFriendProfile(int userId, int page) async {
    final url = buildUrl(appConfig: config, endPoint: "/social/profile/$userId");
    final json = await get(httpClient, url);
    return ProfileDTO.fromJson(json);
  }
